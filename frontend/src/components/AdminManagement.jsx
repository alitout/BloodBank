...
export const AdminManagement = ({ user }) => {
  ...
  // Form states
  const [donorForm, setDonorForm] = useState({ email: "", fname: "", lname: "", phone: "", password: "", bloodType: "O+" });
  ...
  const handleCreateDonor = async (e) => {
    ...
    setMessage("");
    try {
      const result = await authAPI.createDonorByAdmin(
        donorForm.email,
        donorForm.fname,
        donorForm.lname,
        donorForm.phone,
      ...
      if (result.success) {
        ...
        setDonorForm({ email: "", fname: "", lname: "", phone: "", password: "", bloodType: "O+" });
        resetMessage();
      } else {
      ...
      {activeForm === "donor" && (
        <form onSubmit={handleCreateDonor} className="bg-white border border-slate-200 rounded-lg p-6 space-y-4">
          ...
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => { setActiveForm(null); setDonorForm({ email: "", fname: "", lname: "", phone: "", password: "", bloodType: "O+"}); }} className="bg-slate-300 text-slate-700 px-6 py-2 rounded font-semibold hover:bg-slate-400">{t("cancel")}</button>
          </div>
        </form>
      )}
